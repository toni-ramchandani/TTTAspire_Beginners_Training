...
def sync_dataset(project_root: Path, settings: LangSmithSettings) -> dict[str, Any]:
    """Create or update the hosted dataset from the ten governed examples."""
...
def run_live(
    *,
    ...
    prefix = experiment_prefix or f"medical-education-{rag_provider}-top{top_k}"
    kwargs = {
        "data": data,
        ...
        "description": (
            "MediGuide evaluated over the governed ten-case "
...
def compare_experiments(
    *,
    ...
    results = client.evaluate(
        ...
        description=(
            "Risk-first deterministic comparison of two medical patient-education RAG "
...
def reevaluate_existing(
    *, experiment: str, settings: LangSmithSettings, profile: str
    ...
    return client.evaluate(
        experiment,
        evaluators=build_evaluators(profile),
        experiment_prefix="medical-education-added-evaluators",
